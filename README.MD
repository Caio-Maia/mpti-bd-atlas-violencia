...
# 6.1 Estratégia Geral de Modelagem
...
# 6.2 Quadro A — Correspondência Conceito ↔ Dados Coletados
...
# 6.3 Quadro B — Mapeamento para o MongoDB
...
# 6.5 Decisões de Embedding × Referência
...
# 6.7 Alinhamento com os Requisitos de Consulta
...
# 6.8 Padrões de Modelagem Aplicados
...
# 6.9 Coleções Finais do Projeto

Ao final da execução do pipeline ETL o banco MongoDB será composto pelas seguintes coleções:

| Coleção    | Finalidade                        |
| ---------- | --------------------------------- |
| municipios | Dados consolidados da aplicação   |
| ufs        | Catálogo das unidades federativas |
| regioes    | Catálogo das regiões brasileiras  |
| fontes     | Proveniência dos dados            |

A coleção **municipios** será responsável por mais de 90% das consultas executadas pela aplicação e pelo dashboard analítico, enquanto as demais funcionarão como coleções de apoio para enriquecimento semântico, integridade dos dados e consultas com `$lookup`.